...
def ui():
    streamlit.title("Planckian Black-body Temperature to RGB Colorspaces conversion")
    ...
    with streamlit.sidebar.expander(label="Advanced Options"):
        ...
        user_ndecimals = streamlit.number_input(
            label="Number of decimals", min_value=1, max_value=9, value=3, step=1
        )
    ...
    # Processing
    temperature_obj = core.TemperatureObject(
        CCT=user_temperature, tint=user_tint
    ).planckian

    rgb_result = temperature_obj.rgb(
        primaries=user_colorspace, illuminant=user_illuminant, CAT=user_CAT
    )
    ...
    rgb_preview = temperature_obj.rgb(
        "sRGB", illuminant=user_illuminant, CAT=user_CAT
    ...
    image_temp_preview = numpy.full((100, 2048, 3), rgb_preview, dtype=numpy.uint8)
    ...
    streamlit.code(
        body=core.utils.CIExy2String(
            temperature_obj.xy, ndecimals=user_ndecimals
        ).tuple,
        language="text",
    )
